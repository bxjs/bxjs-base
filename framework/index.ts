...
exports.handler = async function (event, context, callback) {
    ...
    // 设置为生产环境 TODO 后续需要区分线上灰度环境
    global['__env__'] = 'prod' // 全局变量仅仅用于放在整个应用生命周期变量
    ...
    let __url__ = _.get(evt, 'headers.__url__', undefined)
    ...
    try {
        ...
        // 获取use-agent请求头部信息
        global['__user_agent__'] = evt['headers'] ? (evt['headers']['user-agent'] ? evt['headers']['user-agent'] : '') : ''
